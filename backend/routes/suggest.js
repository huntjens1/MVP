...
// Behouwt bestaande werking, sanitisert output en garandeert min. 3 vragen.

const express = require("express");
const router = express.Router();
const { emit: pushSuggestions } = require("../streams/suggestionsSSE"); // <— let op: emit
const OpenAI = require("openai");

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const MODEL =
  process.env.ASSIST_MODEL || process.env.SUMMARY_MODEL || "gpt-4o-mini";
...
function cleanText(s) {
  ...
  // strip eventueel JSON/metadata aan het eind
  t = t.replace(/\s*[{[].*[\]}]\s*$/s, "");

  // strip opsommingstekens
  t = t.replace(/^\s*[-*•\d\)\.]+\s*/g, "");
...
function fallbackQuestions(ctx) {
  ...
  if (lc.includes("inlog"))
    out.push("Krijgt u een foutmelding bij het inloggen? Zo ja, welke exacte melding?");
  if (lc.includes("netwerk"))
    out.push("Werkt het op een ander netwerk of via hotspot? Kunt u een ping/gateway-test proberen?");
  if (lc.includes("mail") || lc.includes("email"))
    out.push("Gaat het om de e-mailclient of webmail? En welke omgeving/account?");
...
// Beide paden ondersteunen om base-prefix verschillen op te vangen
router.post(["/api/suggest", "/suggest"], async (req, res) => {
  const t0 = Date.now();
  try {
...
Geef 3–6 KORTE vragen (één per regel).`;
    ...
    if (suggestions.length < 3) {
      suggestions = Array.from(
        new Set([...suggestions, ...fallbackQuestions(transcript)])
      ).slice(0, 6);
    }

    // SSE broadcast
    pushSuggestions(conversation_id, { conversation_id, suggestions });
